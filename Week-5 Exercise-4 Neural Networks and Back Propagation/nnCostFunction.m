...
y_vect= zeros(m,num_labels); %5000x10
for i=1:m
  y_vect(i,y(i)) = 1;
  J = J -y_vect(i,:)*(log(h(i,:)))' -(1-y_vect(i,:))*(log(1-h(i,:)))';
...
for i=1:size(Theta1,1)
  reg1 = reg1 + sum(Theta1(i,2:end).^2);
end

for i=1:size(Theta2,1)
  reg2 = reg2 + sum(Theta2(i,2:end).^2);
end
reg = reg1+reg2;
reg = (lambda/(2*m))*reg;
J += reg;
...
a_3 = a3'; %5000x10
for i=1:m
  delta3 = a_3(i,:)' - y_vect(i,:)'; %10x1
  delta2 = ((Theta2)')*(delta3);
  delta2 = delta2'.*(sigmoidGradient([1,z_2(i,:)]));
  delta2 = delta2(2:end);
  D1=D1+delta2'*a_1(i,:);
  D2=D2+delta3*a_2(i,:);
